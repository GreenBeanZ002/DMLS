...
public final class GriefScanModule extends DMLSModule {
    private static final String PREFIX = "§8[§6DMLS - Griefs§8] §7";
    ...
    private static final Pattern ENTRY = Pattern.compile(
            "([A-Za-z0-9_]{3,16}) (broke|placed) (?:x(\\d+) )?(?:minecraft:)?([a-z0-9_]+)", Pattern.CASE_INSENSITIVE);
    ...
    /** Starts a grief scan. Use * as the IGN to include every player. The command and GUI both call this method. */
    public void submit(MinecraftClient client, String ign, String time, String radius) {
        ...
        if (activeSession != null) {
            ChatUtils.sendTranslatedMessage(client, PREFIX, "dmls.chat.griefs.active");
            return;
        }

        String command = "co lookup" + (allPlayers ? "" : " u:" + cleanIgn)
                + " t:" + cleanTime + " r:" + cleanRadius + " a:block";
        ...
        ChatUtils.sendTranslatedMessage(client, PREFIX, "dmls.chat.griefs.start", cleanTime, cleanRadius, "/dmls griefs cancel");
    }
    ...
    public void cancel(MinecraftClient client) {
        if (activeSession != null) {
            ChatUtils.sendTranslatedMessage(client, PREFIX, "dmls.chat.griefs.cancelled");
            activeSession.report(client);
            activeSession = null;
        } else {
            ChatUtils.sendTranslatedMessage(client, PREFIX, "dmls.chat.griefs.nothing");
        }
    }
    ...
    private final class ScanSession {
        ...
        private void tick(MinecraftClient client) {
            ...
            if (pageMarkerSeen && waitTicks >= PAGE_GAP_TICKS) {
                if (currentPage < totalPages) {
                    ...
                    waitTicks = 0;
                    if (currentPage % 5 == 0) {
                        ChatUtils.sendTranslatedMessage(client, PREFIX, "dmls.chat.griefs.progress", currentPage, totalPages);
                    }
                ...
                if (!dataSeen) {
                    ChatUtils.sendTranslatedMessage(client, PREFIX, "dmls.chat.griefs.no_response");
                } else {
                    report(client);
                }
                activeSession = null;
        ...
        private void handleServerMessage(String text) {
            if (NO_RESULTS.matcher(text).find()) {
                ChatUtils.sendTranslatedMessage(MinecraftClient.getInstance(), PREFIX, "dmls.chat.griefs.no_results");
                activeSession = null;
                return;
            }
            ...
            while (entry.find()) {
                ...
                long count = entry.group(3) == null ? 1 : Long.parseLong(entry.group(3));
                String block = entry.group(4).toLowerCase(Locale.ROOT);
                results.computeIfAbsent(player, key -> new LinkedHashMap<>())
                        .computeIfAbsent(action, key -> new LinkedHashMap<>())
                        .merge(block, count, Long::sum);
                dataSeen = true;
                waitTicks = 0;
            }
        ...
        private void report(MinecraftClient client) {
            String header = Text.translatable("dmls.chat.griefs.header", ign, time, radius).getString();
            ...
            if (results.isEmpty()) {
                ...
                results.entrySet().stream()
                        .sorted(Comparator.comparingLong((Map.Entry<String, Map<String, Map<String, Long>>> entry) ->
                                entry.getValue().values().stream().flatMap(blocks -> blocks.values().stream())
                                        .mapToLong(Long::longValue).sum()).reversed())
                        .forEach(playerEntry -> {
                            Map<String, Map<String, Long>> actions = playerEntry.getValue();
                            if (actions.containsKey("broke")) {
                                ChatUtils.sendClientMessage(client, Text.literal("§8• ").append(ChatUtils.translated(
                                        "dmls.chat.griefs.broke", playerEntry.getKey(), itemsSummary(actions.get("broke")))));
                            }
                            if (actions.containsKey("placed")) {
                                ChatUtils.sendClientMessage(client, Text.literal("§8• ").append(ChatUtils.translated(
                                        "dmls.chat.griefs.placed", playerEntry.getKey(), itemsSummary(actions.get("placed")))));
                            }
                        });
            }
